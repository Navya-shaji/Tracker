...
function ExpenseTracker() {
  ...
  return (
    <div>
      ...
      <h2>Categories</h2>
      <select className="category-selection" onChange={handleCategoryChange} value={selectExpense}>
  ...
  <ul>
    {expense.map((item, index) => (
      <li key={index}>
        <strong>{item.category}:</strong> ${item.amount} on {item.date} - {item.description}
      </li>
    ))}
  </ul>
)}
